...
public class ProcedimentoDaoImpl extends AbstractDao<Procedimento, Long> implements ProcedimentoDao{
	...
	public List<Procedimento> ListarProcedimento(Long id){
		Query query = entityManager.createNativeQuery("SELECT p.id, p.descricao, p.valor, p.convenio_id " + " FROM sismed_convenio c " 
				+ "INNER JOIN sismed_procedimento p ON c.id = p.convenio_id WHERE c.id = ?1 ORDER BY p.descricao ", Procedimento.class).setParameter(1, id);
