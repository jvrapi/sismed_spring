...
public interface TConvenioRepository extends JpaRepository<TConvenio, Long>{
	...
	@Query(value = "SELECT * FROM sismed_tipo_convenio t WHERE t.nome LIKE %:dado% AND convenio_id = :convenio", nativeQuery = true)
	List<TConvenio> ListarPorNome(String dado, Long convenio);
